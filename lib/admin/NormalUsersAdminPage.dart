...
class NormalUsersAdminPage extends StatefulWidget {
  @override
  _NormalUsersAdminPageState createState() => _NormalUsersAdminPageState();
}

class _NormalUsersAdminPageState extends State<NormalUsersAdminPage> {
  ...
  Future<void> deleteUser(String email) async {
    ...
    if (response.statusCode == 200) {
      setState(() {
        normalUsers.removeWhere((user) => user['email'] == email);
      });
    } else {
  ...
  void _startSearch() {
    showSearch(
      context: context,
      delegate: UserSearchDelegate(normalUsers, _launchCaller, confirmDelete),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Utilisateurs Normaux', style: TextStyle(color: Colors.white)),
      ...
      body: normalUsers.isEmpty
          ? Center(child: CircularProgressIndicator())
          : ListView.builder(
        itemCount: normalUsers.length,
        itemBuilder: (context, index) {
          return buildUserCard(normalUsers[index]);
        },
      ),
    );
  }
