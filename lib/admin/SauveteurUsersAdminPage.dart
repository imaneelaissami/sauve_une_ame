...
class SauveteurUsersAdminPage extends StatefulWidget {
  @override
  _SauveteurUsersAdminPageState createState() => _SauveteurUsersAdminPageState();
}

class _SauveteurUsersAdminPageState extends State<SauveteurUsersAdminPage> {
  List<dynamic> sauveteurUser = [];
  ...
  Future<void> fetchSauveteurUser() async {
    final response = await http.get(Uri.parse('http://10.0.2.2:3000/api/users'));

    if (response.statusCode == 200) {
      final data = json.decode(response.body);
      final List<dynamic> filteredUsers =
      data.where((user) => user['userType'] == 'sauveteurUser').toList();

      setState(() {
        sauveteurUser = filteredUsers;
      });
    } else {
  ...
  Future<void> deleteUser(String email) async {
    ...
    if (response.statusCode == 200) {
      setState(() {
        sauveteurUser.removeWhere((user) => user['email'] == email);
      });
    } else {
  ...
  void _startSearch() {
    showSearch(
      context: context,
      delegate: UserSearchDelegate(sauveteurUser, _launchCaller, confirmDelete),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Utilisateurs sauveteurs', style: TextStyle(color: Colors.white)),
      ...
      body: sauveteurUser.isEmpty
          ? Center(child: CircularProgressIndicator())
          : ListView.builder(
        itemCount: sauveteurUser.length,
        itemBuilder: (context, index) {
          return buildUserCard(sauveteurUser[index]);
        },
      ),
    );
  }
